...
export default function Timesheets() {
  ...
  const [dateMode, setDateMode] = useState("week"); // "week" or "range"
  const [rangeStart, setRangeStart] = useState(() => getWeekStart().toISOString().split("T")[0]);
  ...
  const effectiveStart = dateMode === "week" ? weekStart : new Date(rangeStart + "T00:00:00");
  const effectiveEnd = dateMode === "week" ? weekEnd : new Date(rangeEnd + "T23:59:59");
  ...
  const weekEntries = useMemo(() => entries.filter(e => {
    ...
    return e.company === company && d >= effectiveStart && d <= effectiveEnd;
  }), [entries, company, effectiveStart, effectiveEnd]);
  ...
  const weeklyByWorker = useMemo(() => {
    ...
    weekEntries.forEach(e => {
      if (!map[e.workerId]) map[e.workerId] = { name: e.workerName, crew: e.crew, days: {}, total: 0, ot: 0, status: e.status, ids: [] };
      const d = new Date(e.date);
  ...
  const activeProjects = new Set(weekEntries.map(e => e.project).filter(Boolean)).size;
  ...
  const exportExcel = () => {
    ...
    a.download = `HD_Timesheet_${company}_${effectiveStart.toISOString().split("T")[0]}.csv`;
    a.click();
  };

  const tabStyle = (t) => ({
    padding:"9px 18px", fontSize:"13px", fontWeight:"500", cursor:"pointer",
    color: tab===t ? "#2D5016" : "#888", background:"none", border:"none",
    ...
    <div>
      ...
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:"14px", flexWrap:"wrap", gap:"10px" }}>
        <div>
          ...
          <div style={{ fontSize:"12px", color:"#888" }}>
            {dateMode === "week" ? `${formatDate(weekStart)} – ${formatDate(weekEnd)}` : `${rangeStart} → ${rangeEnd}`}
          </div>
        </div>
        <div style={{ display:"flex", gap:"8px", alignItems:"center", flexWrap:"wrap" }}>
          {/* Mode toggle */}
          <div style={{ display:"flex", background:"#fff", border:"0.5px solid #ddd", borderRadius:"8px", overflow:"hidden" }}>
            {[["week","By week"],["range","Date range"]].map(([m,l]) => (
              <button key={m} onClick={() => setDateMode(m)} style={{ padding:"6px 12px", fontSize:"12px", fontWeight:"500", cursor:"pointer", border:"none", background:dateMode===m?"#2D5016":"transparent", color:dateMode===m?"#fff":"#888" }}>{l}</button>
            ))}
          </div>
          {dateMode === "week" ? (
            <div style={{ display:"flex", alignItems:"center", gap:"6px", background:"#fff", border:"0.5px solid #ddd", borderRadius:"8px", padding:"4px 10px" }}>
              ...
              <span style={{ fontSize:"12px", fontWeight:"500", minWidth:"130px", textAlign:"center" }}>{formatDate(weekStart).replace(/, \d{4}/,"")} – {formatDate(weekEnd).replace(/, \d{4}/,"")}</span>
              <button onClick={() => setWeekOffset(w=>w+1)} style={{ background:"none", border:"none", cursor:"pointer", color:"#2D5016", fontSize:"16px", padding:"0 4px" }}>→</button>
            </div>
          ) : (
            <DateRangePicker startDate={rangeStart} endDate={rangeEnd} onChange={handleRangeChange} />
          )}
          <button onClick={exportExcel} style={{ padding:"7px 14px", background:"#fff", border:"0.5px solid #ddd", borderRadius:"8px", fontSize:"12px", cursor:"pointer", fontWeight:"500" }}>Export Excel</button>
          {pendingIds.length > 0 && <button onClick={() => approveAll(pendingIds)} style={{ padding:"7px 14px", background:"#2D5016", color:"#fff", border:"none", borderRadius:"8px", fontSize:"12px", cursor:"pointer", fontWeight:"500" }}>Approve all ({pendingIds.length})</button>}
        </div>
      </div>
      ...
      <div style={{ display:"flex", background:"#fff", border:"0.5px solid #ddd", borderRadius:"8px", overflow:"hidden", width:"fit-content", marginBottom:"14px" }}>
        {["Construction","Landscape"].map(c => (
          <button key={c} onClick={() => setCompany(c)} style={{ padding:"7px 22px", fontSize:"13px", fontWeight:"500", cursor:"pointer", border:"none", background:company===c?(c==="Construction"?"#C45C1A":"#2D5016"):"transparent", color:company===c?"#fff":"#888" }}>{c}</button>
        ))}
      </div>

      {/* KPI Stats — now 6 cards */}
      <div style={{ display:"grid", gridTemplateColumns:"repeat(6,1fr)", gap:"10px", marginBottom:"14px" }}>
        {[
          ["Total hours", totalHrs.toFixed(2)+" hrs", "#2D5016"],
          ["OT hours", totalOT.toFixed(2)+" hrs", "#C45C1A"],
          ["Pending approval", pending, "#C45C1A"],
          ["Approved", approved, "#2D5016"],
          ["Active projects", activeProjects, "#2D5016"],
          ["Workers", new Set(weekEntries.map(e=>e.workerId)).size, "#3d6b1e"],
        ].map(([label, val, color]) => (
      ...
      <div style={{ background:"#fff", borderRadius:"12px", border:"0.5px solid #e5e5e0", overflow:"hidden" }}>
        ...
        {tab === "daily" && <>
          ...
          <div style={{ overflowX:"auto" }}>
            <table style={{ width:"100%", borderCollapse:"collapse", fontSize:"12px" }}>
              ...
              <tbody>
                {filtered.length === 0 && <tr><td colSpan={12} style={{ padding:"24px", textAlign:"center", color:"#aaa", fontSize:"13px" }}>No entries for this period</td></tr>}
                {filtered.map(e => (
          ...
          <div style={{ padding:"9px 16px", fontSize:"12px", color:"#888" }}>
            {filtered.length} entries · Total: <b>{totalHrs.toFixed(2)} hrs</b>
          </div>
        </>}
        ...
        {tab === "weekly" && <div style={{ overflowX:"auto" }}>
          <table style={{ width:"100%", borderCollapse:"collapse", fontSize:"12px" }}>
            ...
            <tbody>
              {weeklyByWorker.length === 0 && <tr><td colSpan={12} style={{ padding:"24px", textAlign:"center", color:"#aaa", fontSize:"13px" }}>No entries for this period</td></tr>}
                ...
                <tr key={i} style={{ borderBottom:"0.5px solid #f0f0ea" }}>
                  ...
                  {days.map(d => (
                    <td key={d} style={{ padding:"8px 8px", textAlign:"center", color:w.days[d]>10?"#C45C1A":"inherit", fontWeight:w.days[d]>10?"500":"400" }}>
                      {w.days[d] ? w.days[d].toFixed(2) : <span style={{color:"#ddd"}}>—</span>}
                    </td>
                  ))}
                  <td style={{ padding:"8px 10px", fontWeight:"600" }}>{w.total.toFixed(2)}</td>
        ...
        {tab === "summary" && <div>
          <div style={{ padding:"12px 16px", fontSize:"12px", color:"#888", borderBottom:"0.5px solid #eee" }}>
            {company} · {dateMode==="week"?`${formatDate(weekStart)} – ${formatDate(weekEnd)}`:`${rangeStart} → ${rangeEnd}`} · Total: <b style={{color:"#2D5016"}}>{totalHrs.toFixed(2)} hrs</b> · OT: <b style={{color:"#C45C1A"}}>{totalOT.toFixed(2)} hrs</b>
          </div>
          <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:"14px", padding:"14px 16px" }}>
            {[["By Project",summaryByProject],["By Crew",summaryByCrew],["By Cost Code",summaryByCode]].map(([title,data]) => (
              <div key={title} style={{ border:"0.5px solid #eee", borderRadius:"8px", overflow:"hidden" }}>
                ...
                {data.map(([label, hrs]) => (
                  <div key={label} style={{ display:"flex", justifyContent:"space-between", padding:"7px 12px", borderBottom:"0.5px solid #f5f5f0", fontSize:"12px" }}>
                    ...
                    <span style={{ fontWeight:"500" }}>{hrs.toFixed(2)} hrs</span>
                  </div>
                ))}
              </div>
            ))}
